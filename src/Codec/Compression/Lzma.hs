  ...
  where
    ...
    go ls = return inputRequired
      where
        ...
        goInput chunk = do
            ...
            case rc of
                LzmaRetOK
                  | BS.null obuf -> do
                      unless (used > 0) $
                          error "compressIO: input chunk not consumed"
        ...
        goSync LzmaRun _ = error "goSync called with invalid argument"
    ...
    go ls = return inputRequired
      where
        ...
        goSync action next = goSync'
          where
            goSync' = do
                rc_buf <- strictToLazyST (noDuplicateST >>
                                                 runLzmaStream ls BS.empty action bUFSIZ)
                let (rc, 0, obuf) = rc_buf
                case rc of
                    LzmaRetOK
                        | BS.null obuf -> error ("compressIO: empty output chunk during " ++ show action)
                        | otherwise    -> return (CompressOutputAvailable obuf goSync')
          ...
          | otherwise = do
            ...
            case rc of
                LzmaRetOK
                  | BS.null obuf -> do
                      unless (used > 0) $
                          error "decompressIO: input chunk not consumed"
                      withChunk (return inputRequired) goInput chunk'
